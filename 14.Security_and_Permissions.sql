...
-- Question 8: How can you approximate the correlation between order amount and product price?
-- Explanation: The Pearson correlation formula is implemented using aggregated sums.
-- Answer:
SELECT 
    (COUNT(*) * SUM(o.order_amount * p.price) - SUM(o.order_amount) * SUM(p.price)) /
    SQRT((COUNT(*) * SUM(o.order_amount * o.order_amount) - POW(SUM(o.order_amount), 2)) *
...
-- Question 9: How do you compute z-scores for order amounts?
-- Explanation: Z-scores standardize values using (value - mean) / STDDEV_POP.
-- Answer:
SELECT 
    order_id,
    order_amount,
...
-- Question 10: How can you calculate variance of order amounts by customer city?
-- Explanation: GROUP BY with VARIANCE_POP computes city-level variance.
-- Answer:
SELECT 
    c.city,
    VARIANCE_POP(o.order_amount) AS variance_order_amount
...
-- Question 11: How do you calculate a 7-day rolling standard deviation of order amounts?
-- Explanation: STDDEV_POP with a ROWS window computes variability over 7 days.
-- Answer:
SELECT 
    order_id,
    order_date,
...
-- Question 12: How can you analyze variance of order amounts by customer cohort?
-- Explanation: A CTE defines cohorts, and VARIANCE_POP computes variability per cohort.
-- Answer:
WITH first_orders AS (
    SELECT 
        customer_id,
...
-- Question 13: How do you generate a statistical summary for order amounts?
-- Explanation: Combine COUNT, AVG, STDDEV_POP, VARIANCE_POP, MIN, and MAX in one query.
-- Answer:
SELECT 
    COUNT(*) AS total_orders,
    AVG(order_amount) AS mean_amount,
...
-- Question 14: How can you calculate the percentile rank for each order amount?
-- Explanation: CUME_DIST assigns a percentile rank based on order_amount.
-- Answer:
SELECT 
    order_id,
    order_amount,
...
-- Question 15: How do you approximate correlation within product categories?
-- Explanation: GROUP BY category applies the Pearson correlation formula per category.
-- Answer:
SELECT 
    p.category,
    (COUNT(*) * SUM(o.order_amount * p.price) - SUM(o.order_amount) * SUM(p.price)) /
...
-- Final Notes
-- =====================================
-- - Statistical functions like STDDEV_POP and VARIANCE_POP are essential for understanding data distribution.
-- - Percentile and median calculations in MySQL require approximations using window functions (MySQL 8.0+).
-- - Correlation calculations are approximations; for precise statistics, consider external tools like Python or R.
-- - Optimize performance with indexes on order_amount and product_id for large datasets.
-- - Ensure the `mysql_learning` database is set up with sample datasets (customers.csv, orders.csv, products.csv, sales.csv).
-- - For further learning, explore `Time_Intelligence.sql` for temporal analytics or `Advanced_Queries.sql` for complex query patterns.
